...
Figures (thirteen; figs 1-5 Session 1, 6-9 Session 2, 10-13 Session 3) are
generated by `slides/figures.py` (run with `template/.venv/bin/python`)
into `slides/figures/`, in the deck palette, and passed to `figureSlide` by
path. Fig 5 imports the template's real `core.spectrum` — its spikes are
...
Untrimmed, Fig 11's annotation labels were borderline unreadable from a back
